...
if (isset($_POST['submitBtn'])) {
    ...
    if ($image_type == "image/webp" || $image_type == "image/png" || $image_type == "image/jpg") {
        if ($image_size <= 1000000) { //1000000 KB = 1MB
            ...
            $sql = "INSERT INTO `jewellery`(`name`, `image`, `price`, `description`, `category`) VALUES ('$name','$folder','$price', '$description','$category')";
            $query = mysqli_query($con, $sql);
            if ($query) {
                move_uploaded_file($image_tmp_name, $folder);
                $_SESSION["successMessage"] = "Category Added Successfully!!";
                echo "<script>window.location.href='jewellery.php'</script>";
            } else {
    ...
    <div class="row justify-content-center">
        <div class="col-8 card shadow">
            <form action="add_jewellery.php" method="post" enctype="multipart/form-data">
                <div class="my-3">
                    <h1 class="text-primary text-center text-uppercase">Add Jewellery</h1>
                </div>
                ...
                <div class="my-3">
                    ...
                    <input type="number" step="any" name="price" id="" class="form-control">
                </div>

...
<div class="video-banner-container" style="width: 100%; position: relative; height: 700px; overflow: hidden;">
    <video autoplay muted loop playsinline style="width: 100%; height: 100%; object-fit: cover; display: block;">
        <source src="videos/cosmetic_banner.mp4" type="video/mp4">
    ...
    <div class="container">
        <div class="flex-w flex-sb-m p-b-52">
            <div class="flex-w flex-l-m filter-tope-group m-tb-10">
                ...
                <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".skin_care">
                    Skin Care
                </button>

                <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".hair_care">
                    Hair Care
                </button>

                <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".fragrances">
                    Fragrances
                </button>

                <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".body">
                    Bath&Body
                </button>

                <button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".make_up">
                    Make Up
                </button>
            </div>
        ...
        <div class="row isotope-grid" id="product-grid">
            ...
            while ($row = mysqli_fetch_array($run)) {
                $product_count++;
                // Determine category class based on product category or name
                $category_class = '';
                if (isset($row[5])) {
                    // If you have a category column (column 5)
                    $category = strtolower($row[5]);
                } else {
                    // If no category column, try to determine from product name
                    $product_name = strtolower($row[1]);
                    if (strpos($product_name, 'cream') !== false || strpos($product_name, 'serum') !== false || strpos($product_name, 'cleanser') !== false || strpos($product_name, 'moisturizer') !== false) {
                        $category = 'skin_care';
                    } elseif (strpos($product_name, 'shampoo') !== false || strpos($product_name, 'conditioner') !== false || strpos($product_name, 'hair') !== false) {
                        $category = 'hair_care';
                    } elseif (strpos($product_name, 'perfume') !== false || strpos($product_name, 'cologne') !== false || strpos($product_name, 'fragrance') !== false) {
                        $category = 'fragrances';
                    } elseif (strpos($product_name, 'body') !== false || strpos($product_name, 'bath') !== false || strpos($product_name, 'soap') !== false) {
                        $category = 'body';
                    } elseif (strpos($product_name, 'lipstick') !== false || strpos($product_name, 'foundation') !== false || strpos($product_name, 'mascara') !== false || strpos($product_name, 'makeup') !== false) {
                        $category = 'make_up';
                    } else {
                        $category = 'general';
                    }
                }
                $category_class = str_replace(' ', '_', strtolower($category));
            ?>
                <div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item product-item <?php echo $category_class; ?>" data-category="<?php echo $category_class; ?>">
                    <div class="block2">
        ...
        <?php if (!empty($search_query)): ?>
            <div class="flex-c-m flex-w w-full p-t-20 p-b-20">
                <div class="stext-102 cl6">
                    <?php if ($product_count > 0): ?>
                        Found <?php echo $product_count; ?> product(s) matching "<?php echo htmlspecialchars($search_query); ?>"
                    <?php else: ?>
                        No products found matching "<?php echo htmlspecialchars($search_query); ?>". Try different keywords.
                    <?php endif; ?>
                </div>
            </div>
        <?php endif; ?>
